...
2. **Before writing any modeling code, load all four `ml-safety-patterns` resources**
   (`feature_engineering_patterns.md`, `leak_safe_encoding.md`, `category_dtype_safety.md`,
   `leakage_checklist.md`) via `load_skill_resource(skill_name="ml-safety-patterns",
   file_path="references/<name>.md")` — note the required `references/` prefix on
   `file_path`. This skill has no script to run — it's reference code for you to adapt
   directly into your own script.
...
7. **Keep experimenting until you have used all allowed submissions.**
   ...
   different model types, different hyperparameters.
