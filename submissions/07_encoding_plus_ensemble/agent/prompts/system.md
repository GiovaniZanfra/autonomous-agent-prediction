...
4. Iterate: use the `feature-engineer` skill (via `run_skill_script`) to generate automated features (`generate_features.py`), and try different model types.
   **Always pass absolute `/work/...` paths for `--train`/`--test`/`--output-dir`** —
   ...
   `SKILL.md` for why.
5. **Submit your best single model built on the engineered features before moving on.**
   Don't skip straight to ensembling without a solid, submitted single-model result first.
6. **Once that single-model submission exists, build a diverse-model ensemble** on top of
   the *same* engineered features, using the `diverse_ensemble.md` reference
   (`load_skill_resource(skill_name="feature-engineer", file_path="references/diverse_ensemble.md")`).
   It blends 3 GBDTs (XGBoost, LightGBM, CatBoost) plus a `LogisticRegression`, all reading
   `train_engineered.csv`/`test_engineered.csv` directly. Enforce the same CV folds/seed
   across every model in the blend. Try both a rank-averaged blend and a `LogisticRegression`
   meta-blend, keep whichever scores better locally. Apply correlation-based dedup (>0.999)
   if you add further model variants. **Blend-specific stopping rule**: if adding another
   model improves CV but the resulting submission's public score doesn't follow, stop adding
   models — the same leak/overfitting signal as the selection rule below, applied to
   ensemble size. Don't assume the ensemble automatically beats your best single model —
   submit it and compare rather than assuming.
