...
### `diverse_ensemble.md`
The Recipe 2 pattern: 3-4 genuinely diverse models (3 GBDTs + one linear model) blended via
OOF rank-transform, with the same-folds discipline, correlation-based dedup, and a
blend-specific stopping rule. Build this *after* a single-model baseline is already
submitted — see `system.md`'s workflow for when.
